...
function sliderInit(slider) {
  ...
  slider.html(container); // Créer un élément nav

  var nav = $('<nav></nav>'); // Dans cet élément, ajouter un bouton .prev et un bouton .next

  nav.append('<button class="prev"></button>');
  nav.append('<button class="next"></button>'); //Ajouter au slider
...
function slide(slider) {
  ...
  var currentSlide = Number(slider.attr('data-currentSlide')); //- modifier le left du container de slides (en css : mettre le container de slides en position relative)
  ...
  if (currentSlide == -1) {
    ...
    slider.children('.slides-container').on('transitionend', function () {
      // Retirer l'écouteur 
      $(this).off('transitionend'); // Retour à la dernière image

      var lastIndex = $(this).find('.slide').length - 2;
      slider.attr('data-currentSlide', lastIndex); // Retirer la transition

      $(this).css('transition', 'none');
      slide(slider);
      setTimeout(function () {
        enableNav(slider); // Remettre la transition

        slider.children('.slides-container').css('transition', 'left 1s'); // Retirer le clone mis avant l'image 0
  ...
  if (currentSlide == slider.find('.slide').length) {
    ...
    slider.children('.slides-container').on('transitionend', function () {
      // Retirer l'écouteur 
      $(this).off('transitionend'); // Retour à la première image

      slider.attr('data-currentSlide', 0); // Retirer la transition
      ...
      setTimeout(function () {
        ...
        slider.children('.slides-container').css('transition', 'left 1s'); // Retirer le clone mis après la dernière image
